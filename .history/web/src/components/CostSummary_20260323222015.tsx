...
function Row({
  ...
  return (
    <div
      className="flex justify-between gap-4 py-[3px]"
      style={{ fontWeight: bold ? 700 : 400 }}
    >
      <span
        className="whitespace-nowrap"
        style={{ color: muted ? "#888" : undefined }}
      >
        {left}
      </span>
      <span className="whitespace-nowrap text-right">{right}</span>
    </div>
  );
}
...
export default function CostSummary({ config, result }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Monthly total */}
      <Row left="TOTAL MÊS" right={formatCurrency(result.totalMonthlyCost)} bold />

      <Sep />

      {/* Projection */}
      <Row left="PROJEÇÃO 2026" right={formatCurrency(result.projection2026)} bold />
      <div
